...
import { getProductByFilter, getProductsAndCategory } from "../../../services/adminService";
import { IProduct } from "../../../interface/IProduct";

interface IProps {
    setAddProductModal: any;
    selectedTab: string;
    setCategories: any;
    categories: any;
}

function AddProductInCategoryModal({ categories, setAddProductModal, selectedTab, setCategories }: IProps) {
    const [products, setProducts] = useState<IProduct[]>([]);
    const [search, setSearch] = useState("");
    const [filter, setFilter] = useState("");

    const addProduct = (obj: any) => {
        setCategories((prev: any) => ({
            ...prev,
            [selectedTab]: [...prev[selectedTab], obj],
        }));
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                ...
                <div className="px-6 py-4 border-b flex justify-between items-center">
                    ...
                    <button onClick={() => setAddProductModal(false)} className="text-gray-400 hover:text-gray-500">
                          ...
                          >
                            ...
                            <div className="p-4 space-y-1">
                              <h4 className="text-lg font-semibold text-gray-800 truncate">
                                {obj?.name || "Unnamed Product"}
                              </h4>
                            </div>
                          
                            {/* Add/Remove Button */}
                            <div className="px-4 pb-4">
                              {categories[selectedTab]?.find((data: any) => data?._id === obj?._id) ? (
                                <button
                ...
                {
                    products?.length==0&&   <p className="text-sm text-gray-500 col-span-2 py-4 text-center">
...
export default AddProductInCategoryModal;
